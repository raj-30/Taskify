...
import { auth, googleProvider, signInWithPopup, signOut, app } from "./firebaseAuth.js";
    ...
    signInButton.addEventListener("click", () => {
        signInWithPopup(auth, googleProvider)
            .then((result) => {
                ...
                const email = error.customData && error.customData.email ? error.customData.email : 'N/A';
                console.error("Google Sign-In Error:", errorCode, errorMessage, "Email:", email);
...
function appendItemToShoppingListEl(item) {
    ...
    let userTag = itemData.userName ? `<span class="user-tag">@${itemData.userName.split(' ')[0]}</span>` : '';
    let deadlineDisplay = itemData.deadline ? `<span class="deadline">Due: ${itemData.deadline}</span>` : '';
